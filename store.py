...
Layout:
    data/index/<user>/
        chunks.jsonl     one JSON object per line: {file_id, filename, text}
        vectors.npy      (N, 384) float32, L2-normalised
        meta.json        {model, dim, count, updated_at}

The store is a derived artefact — it can be rebuilt from raw files.
"""
# Bug fix: removed unused `import pickle`
